...
現在の公開版は **2.0.0-beta.43** です。開発中のベータ版であるため、本番配信へ導入する前に、利用環境で受信、再接続、OBS出力、音声、取消・解除を十分に確認してください。CDI-Telopperは気象庁の公式ソフトではありません。
本ソフトウェアだけを防災判断の根拠にせず、必ず気象庁などの公式情報も確認してください。

- [2.0.0-beta.43をダウンロード](https://github.com/black-hawk-bhd/CDI-Telopper-beta/releases/tag/v2.0.0-beta.43)
- [詳細README・操作説明・仕様書](README_CDI-Telopper_2.0.0-beta.43.txt)
- [ソースからのビルド方法](SOURCE_BUILD.md)
- [開発者向けコードガイド](docs/DEVELOPER_GUIDE.md)

## 主な機能

地図は開発途中のため、通常ビルド・配布ビルドでは無効です。関連ボタンはグレー表示され、地図コード・境界データ・テストのみ保持しています。[開発用の説明](docs/trial-seismic-map.md)。beta.43の配布版も地図機能は無効です。

beta.43: 気象字幕は情報種別のみをバッジにし、都道府県・状態を隣、市町村一覧を下に表示します。津波観測情報の案内と警報・注意報の発表状況を別ページにしました。

beta.42: 旧電文VPWW53・VPWW54・VPOA50を気象庁XMLで除外し、AXISでもVPOA50を除外。気象字幕を共通見出し＋地域一覧・状態別ページに変更し、電文確認画面に種別フィルターを追加しました。
beta.41では気象庁XMLの受信電文が解析前に除外される不具合を修正しました。受信元に「気象庁XML」を選択できます。設定の受信元で手動選択し「保存して反映」を押してください。EEW以外の既存対応情報を、公式の高頻度フィードから60秒以上の間隔で取得します。認証は不要です。接続前の電文は新着として表示しません。掲載遅延・停止があり、長時間の受信停止中の電文をすべて補完する機能ではありません。AXIS・P2P障害の自動判定・自動切替は行いません。[気象庁の利用上の注意](https://xml.kishou.go.jp/xmlpull.html)も確認してください。
...
- 情報分類ごとにP2P地震情報、DMDATA.JP、AXIS、Wolfx、気象庁XML（EEW以外）、または「受信しない」を選択
- 選択された情報に不要なプロバイダーAPIへは接続しない構成
- 同一イベントの続報、取消、解除、重複、古い更新報を考慮した表示制御
- 注意報・警報解除時に対象地域と解除種別を表示
- 本番受信電文と過去電文を確認できる独立ウインドウ（種別フィルター・該当件数付き。配信側の設定は変更しません）
...
| 気象 | VPWW55～61、VPWS50、VPBS50・51、VPHW50・51、VXKO50～89（受信元による） |
| 火山 | VFVO50、VFVO56 |
| 南海トラフ | VYSE50 |

VPOA50の解析機能は過去電文等のために残していますが、気象庁XML・AXISの新着受信では除外します。DMDATAの受信設定は変更していません。
...
気象庁が公開するXML電文を直接取得する、認証不要の受信元です。
...
試験的プロバイダーです。APIは気象庁が運営する公式APIではありません。気象庁発表情報を扱う外部サービスとして、公式情報と併用してください。利用者自身が取得した有効なアクセストークンと、利用するチャンネルの契約が必要です。`eew`、`jmx-seismology`、`jmx-meteorology`、`jmx-volcanology`を、選択した情報分類に応じて使用します。
...
このスクリプトは依存関係を復元し、Release構成で全プロジェクトをビルドして、自動テストを実行します。現在のbeta.43では555件のテストを確認しています。
...
フォルダ版、単一EXE版、`version.json`、`SHA256SUMS.txt`が`artifacts\release\2.0.0-beta.43\win-x64`へ生成されます。詳しくは[SOURCE_BUILD.md](SOURCE_BUILD.md)を参照してください。
