...
Beta.44 moves disaster-prevention bulletin area headings beside the badge, including continuation pages. River-flood narrative badges and colors follow each explicitly stated station alert level (2–5). Experimental maps support municipality markers, all-point rendering and built-in test telegrams; maps remain disabled in public packages.
...
The experimental map is disabled in normal and distribution builds. Its buttons remain disabled; source, geographic data and tests are retained for development. Beta.44 distribution packages also disable the map.
...
The current public release is **2.0.0-beta.44**. This is a development beta. Before using it in a live broadcast, thoroughly test reception, reconnection, OBS output, audio, cancellations, and the lifting of warnings and advisories in your own environment. Do not rely on this application as your sole source for safety decisions. Always confirm critical information through official sources such as the Japan Meteorological Agency (JMA).

- [Download 2.0.0-beta.44](https://github.com/black-hawk-bhd/CDI-Telopper-beta/releases/tag/v2.0.0-beta.44)
- [Detailed Japanese manual and specification](README_CDI-Telopper_2.0.0-beta.44.txt)
...
The script restores dependencies, builds every project in the Release configuration, and runs the automated tests. The beta.44 source currently has 558 verified tests.
...
The folder package, single-file package, `version.json`, and `SHA256SUMS.txt` are written to `artifacts\release\2.0.0-beta.44\win-x64`. See [SOURCE_BUILD.md](SOURCE_BUILD.md) for details.
